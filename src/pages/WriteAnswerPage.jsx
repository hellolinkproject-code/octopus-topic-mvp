...
export default function WriteAnswerPage() {
  ...
  const submit = async () => {
    ...
    if (length < writingPrompt.minCharacters || length > writingPrompt.maxCharacters) {
      setError(t(isEssay ? 'writing.lengthError54' : 'writing.lengthError'))
      return
    }
    setSaving(true)
    try {
  ...
  return (
    <Layout>
      <main className="writing-page page-width">
        <button className="back-link standalone" onClick={() => navigate(path('/writing'))}>
          <ArrowLeft size={16} /> {t('writing.backToLab')}
        </button>
        <div className="writing-header">
          <div>
            <span>{t('common.writing')}</span>
            <h1>
              <b>{number}</b> {t(isEssay ? 'writing.title54' : 'writing.title')}
            </h1>
            <p>{t(isEssay ? 'writing.subtitle54' : 'writing.subtitle')}</p>
          </div>
          <div className="autosave">
            <CheckCircle2 size={16} /> {draftSaved ? t('writing.saved') : t('writing.autosave')}
          </div>
        </div>
        <div className="writing-grid">
          {!isEssay ? (
            ...
            <Card className="prompt-card essay-prompt">
              <span className="prompt-label">{t('writing.essay')}</span>
              <h2>{writingPrompt.title}</h2>
              <p className="essay-topic">{writingPrompt.topic}</p>
              <p>{writingPrompt.description}</p>
              <ol>
                {writingPrompt.questions.map((q) => (
                  <li key={q}>{q}</li>
          ...
          <Card className="editor-card">
            <div className="editor-top">
              <div>
                ...
                <small>{isEssay ? t('writing.recommended54') : t('writing.recommended')}</small>
              </div>
              <span
                className={
            ...
            <Textarea
              ...
              aria-label={`${number}${t(isEssay ? 'writing.title54' : 'writing.title')}`}
              placeholder={t(isEssay ? 'writing.placeholder54' : 'writing.placeholder')}
              value={content}
        ...
        {savedAnswer ? (
          <section className="writing-feedback" ref={feedbackRef} aria-live="polite">
            ...
            <div className="feedback-grid">
              <Card className="feedback-answer">
                <span>{t('writing.mySavedAnswer')}</span>
                <article>{savedAnswer.content}</article>
              </Card>
              {!isEssay ? (
                <Card className="feedback-answer model">
                  <span>{t('writing.modelAnswer')}</span>
                  <article>{feedback.modelAnswer}</article>
                </Card>
              ) : (
                <Card className="feedback-answer model outline-card">
                  <span>{t('writing.outline')}</span>
                  <ol>
                    {feedback.outline.map((item) => (
                      <li key={item}>{item}</li>
                    ))}
                  </ol>
                </Card>
            ...
            <Card className="feedback-explanation">
              <div>
                <Lightbulb />
                <h3>{t('writing.explanation')}</h3>
              </div>
              <ul>
            ...
            <Card className="premium-feedback-card">
            ...
            <div className="feedback-actions">
              ...
              <Button onClick={() => navigate(path('/answers'))}>
                <ListChecks size={17} />
                {t('writing.viewAnswers')}
              </Button>
            </div>
          </section>
        ) : null}
