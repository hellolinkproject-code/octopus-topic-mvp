export function calculateQuizResult(questions, selections) {
  const correctCount = questions.reduce(
    (count, q, i) => count + (selections[i] === q.answer ? 1 : 0),
    0,
  )
  return {
    correctCount,
...
export function awardQuiz(state, quizId, result) {
  if (state.completedQuizIds.includes(quizId))
    return { ...state, latestQuizResult: { ...result, awarded: false } }
  return {
    ...state,
    points: state.points + result.earnedPoints,
    completedQuizIds: [...state.completedQuizIds, quizId],
    latestQuizResult: { ...result, awarded: true },
  }
}
