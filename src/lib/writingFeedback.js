const joinValues = (prompt, seriesKey) =>
  prompt.chartData
    .map(
      (group) => `${group.label} ${group.values.find((item) => item.series === seriesKey).value}%`,
    )
    .join(', ')

export function getWritingFeedback(prompt) {
  if (isEssayTask(prompt.number))
    return {
  ...
  const byDifference = [...comparisons].sort(
...
import { isEssayTask } from './writingTask'
